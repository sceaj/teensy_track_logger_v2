...
static void SDMMCHOST_DetectCardInsertByHost(USDHC_Type *base, void *userData)
{
    ...
    SDMMC_LOG("SDMMCHOST_DetectCardInsertByHost\n");
...
static void SDMMCHOST_DetectCardRemoveByHost(USDHC_Type *base, void *userData)
{
    ...
    SDMMC_LOG("SDMMCHOST_DetectCardRemoveByHost\n");
...
status_t SDMMCHOST_CardDetectInit(sdmmchost_t *host, void *cd)
{
    ...
    {
    	SDMMC_LOG("Calling USDHC_CardDetectByData3\n");
    ...
    USDHC_EnableInterruptSignal(base, kUSDHC_CardInsertionFlag | kUSDHC_CardRemovalFlag);

    if (SDMMCHOST_CardDetectStatus(host) == kSD_Inserted)
    {
    	SDMMC_LOG("SDMMCHOST_CardDetectStatus = kSD_Inserted\n");
    ...
    {
    	SDMMC_LOG("SDMMCHOST_CardDetectStatus = kSD_Removed\n");
...
{
    return ((USDHC_GetPresentStatusFlags(host->hostController.base) & kUSDHC_CardInsertedFlag))
    		? kSD_Inserted
    		: kSD_Removed;
}
...
status_t SDMMCHOST_PollingCardDetectStatus(sdmmchost_t *host, uint32_t waitCardStatus, uint32_t timeout)
{
    ...
    SDMMC_LOG("SDMMC_OSAEventGet: 0x%X  waitCardStatus: %d\n", event, waitCardStatus);
    if ((((event & SDMMC_OSA_EVENT_CARD_INSERTED) == SDMMC_OSA_EVENT_CARD_INSERTED) && (waitCardStatus == kSD_Inserted))
    		|| (((event & SDMMC_OSA_EVENT_CARD_REMOVED) == SDMMC_OSA_EVENT_CARD_REMOVED) && (waitCardStatus == kSD_Removed)))
    {
    	if (SDMMCHOST_CardDetectStatus(host) == waitCardStatus) {
			SDMMC_LOG("SDMMCHOST_PollingCardDetectStatus got expected event\n");
			return kStatus_Success;
    	}
    }
    ...
    do
    {
    	SDMMC_LOG("SDMMCHOST_OSAEventWait...\n");
