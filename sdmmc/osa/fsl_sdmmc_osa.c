...
status_t SDMMC_OSAEventSet(void *eventHandle, uint32_t eventType)
{
    ...
    if (eventType > 0xFF)
    	PRINTF("SDMMC_OSAEventSet: eventHandle=0x%X  eventType=0x%X\n", eventHandle, eventType);
...
status_t SDMMC_OSAEventGet(void *eventHandle, uint32_t eventType, uint32_t *flag)
{
    ...
    if (eventType > 0xFF)
    	PRINTF("SDMMC_OSAEventGet: eventHandle=0x%X  eventType=0x%X return=0x%X\n", eventHandle, eventType, *flag);
...
status_t SDMMC_OSAEventClear(void *eventHandle, uint32_t eventType)
{
    ...
    if (eventType > 0xFF)
    	PRINTF("SDMMC_OSAEventClear: eventHandle=0x%X  eventType=0x%X\n", eventHandle, eventType);
